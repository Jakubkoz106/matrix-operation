def inputaADD():
    q = [int(a) if a.isdigit() else float(a) for a in input("Enter size of first matrix (2 arguments) :").split()]
    while len(q) !=2:
    ...
    while flaga:
        flaga = False
        A = [[int(a) if a.isdigit() else float(a) for a in input("Enter first matrix row nr "+str(i+1)+":").split()] for i in range(q[0])]
        for i in range(q[0]):
            if len(A[i]) != q[1]:
                flaga = True
        if flaga:
    ...
    while flaga:
        flaga = False
        B = [[int(a) if a.isdigit() else float(a) for a in input("Enter second matrix row nr "+str(i+1)+":").split()] for i in range(w[0])]
        for i in range(w[0]):
            if (len(B[i]) != w[1]):
                flaga = True
        if flaga:
...
def inputaMULTIPLY():
    ...
    while flaga:
        flaga = False
        A = [[int(a) if a.isdigit() else float(a) for a in
              input("Enter first matrix row nr " + str(i + 1) + ":").split()] for i in range(q[0])]
        for i in range(q[0]):
            if len(A[i]) != q[1]:
                flaga = True
        if flaga:
...
def inputaMULTIPLYTWO():
    ...
    while flaga:
        flaga = False
        B = [[int(a) if a.isdigit() else float(a) for a in
              input("Enter second matrix row nr " + str(i + 1) + ":").split()] for i in range(w[0])]
        for i in range(w[0]):
            if (len(B[i]) != w[1]):
                flaga = True
        if flaga:
